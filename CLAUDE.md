...
2. **Un seul projet Google Cloud partagé pour YouTube** — le quota (10 000 units/jour) est mutualisé par projet, pas par chaîne, donc le contrôle bloquant dans `/api/schedule` se fait sur le **total projet toutes orgs confondues** (`getYouTubeQuotaUsageGlobal`), pas sur le compteur d'une seule org (`getYouTubeQuotaUsage` reste dispo pour le reporting par client). Le coût d'un upload est **réservé au moment de la planification** (pas à la publication réelle : il n'existe pas encore de worker de publication YouTube dans ce repo). La journée de quota est découpée sur le **fuseau Pacifique** (`todayPacificDateKey`), qui correspond au vrai reset quotidien de YouTube — voir `lib/youtube/quota.ts`
3. **Rate limit TikTok** : max 6 req/min → toujours passer par la queue. Exception assumée : `/api/tiktok/creator-info` appelle TikTok en synchrone à l'ouverture du composer / au changement de compte (1 appel par action utilisateur, jamais en bulk) — volume négligeable, tradeoff accepté, pas un oubli
...
- Publication TikTok : le code ne force plus `SELF_ONLY` — le composer envoie désormais la valeur réellement choisie par l'utilisateur, validée côté serveur contre `privacy_level_options` renvoyé par `creator_info`. En revanche, TikTok force toujours `SELF_ONLY` de son côté pour toute app non auditée, quelle que soit la valeur envoyée : le comportement effectif (posts privés uniquement) est donc inchangé, seule la raison a changé. Le posting public s'ouvrira automatiquement une fois l'audit approuvé, sans modification de code
